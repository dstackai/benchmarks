...
required_columns = [gpu_col, qps_col, completed_col]
...
valid_qps = df.groupby(qps_col)[completed_col].count()
...
plt.title('Completed Requests vs QPS for Different GPUs', fontsize=18)
plt.xlabel('QPS', fontsize=14)
plt.ylabel('Completed Requests', fontsize=14)
...
# Save the plot
plt.savefig('images/completed_requests_vs_qps_comparison_gpus.png', dpi=300, bbox_inches='tight')
